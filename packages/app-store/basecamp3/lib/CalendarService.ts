...
class BasecampCalendarService implements Calendar {
  ...
  private async getBasecampDescription(event: CalendarEvent): Promise<string> {
    ...
    const baseString = `<div>Event title: ${event.title}<br/>Date and time: ${date}, ${startTime} - ${endTime} ${timeZone}<br/>View on teeth.al: <a target="_blank" rel="noreferrer" class="autolinked" data-behavior="truncate" href="https://app.cal.com/booking/${event.uid}">https://app.cal.com/booking/${event.uid}</a> `;
