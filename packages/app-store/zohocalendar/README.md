...
2. Create a "Server-based Applications", set the Redirect URL for OAuth `<teeth.al URL>/api/integrations/zohocalendar/callback` replacing [teeth.al](https://cal.com) URL with the URI at which your application runs.
3. Fill in any information you want in the "Client Details" tab
4. Go to tab "Client Secret" tab.
5. Now copy the Client ID and Client Secret into your app keys in the [teeth.al](https://cal.com) admin panel (`<teeth.al>/settings/admin/apps`).
...
9. You're good to go. Now you can easily add your Zoho Calendar integration in the [teeth.al](https://cal.com) settings at `/settings/my-account/calendars`.
