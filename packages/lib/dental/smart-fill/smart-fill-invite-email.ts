...
export function buildSmartFillInviteEmail(params: InviteEmailParams) {
  ...
  const practice = params.practiceName ?? "Ihrer Zahnarztpraxis";
  const startFormatted = dayjs(params.slotStart).tz(params.timeZone).format("dddd, DD.MM.YYYY [um] HH:mm");
...
<body style="font-family:Arial,sans-serif;max-width:560px;margin:0 auto;padding:24px;color:#333;">
  <p>Guten Tag ${params.patientName},</p>
  <p>in <strong>${practice}</strong> ist am <strong>${startFormatted}</strong> ein Termin frei geworden (${treatment}).</p>
  <p style="margin:28px 0;">
    <a href="${confirmUrl}" style="background:#0d9488;color:#fff;padding:12px 20px;border-radius:8px;text-decoration:none;font-weight:bold;margin-right:12px;">Termin bestätigen</a>
