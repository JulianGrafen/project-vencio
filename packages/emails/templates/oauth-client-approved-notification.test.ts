  ...
  it("renders a node mailer payload containing the approved client information", async () => {
    const t = ((key: string, vars?: Record<string, unknown>) => {
      ...
      if (key === "oauth_client_approved_email_footer") {
        return "You can now use your client ID and secret to integrate with teeth.al.";
      }
    ...
    expect(html).toContain("You can now use your client ID and secret to integrate with teeth.al.");
  });
});
